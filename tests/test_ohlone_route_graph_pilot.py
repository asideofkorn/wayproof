...
def test_pilot_changeset_is_validated_and_exactly_accounts_for_the_slice():
    ...
    assert len(change.operations) == 269
    assert len({item.path for item in change.operations}) == 269
...
def test_official_mainline_is_connected_from_stanford_to_lichen_bark(snapshot):
    endpoints = {}
    for item in snapshot.relationships:
        if not item.subject_id.startswith("route-segment-ohlone-"):
            continue
        if item.predicate in {"starts_at", "ends_at"}:
            endpoints.setdefault(item.subject_id, {})[item.predicate] = item.object_id

    official = ["route-segment-ohlone-stanford-avenue-ot1"]
    official.extend(
        f"route-segment-ohlone-ot{number}-ot{number + 1}"
        for number in range(1, 28)
        if number not in {26}
    )
    official[26:26] = ["route-segment-ohlone-ot26-ot27"]
    official.extend([
    ...
    official.append("route-segment-ohlone-lichen-bark-ot40")

    current = "staging-mission-peak-stanford-avenue"
    for segment_id in official:
        assert endpoints[segment_id]["starts_at"] == current
        current = endpoints[segment_id]["ends_at"]
    assert current == "trailhead-ohlone-lichen-bark"


def test_complete_topology_does_not_invent_unverified_edge_mileage(snapshot):
    gap = next(item for item in snapshot.gaps
               if item.gap_id == "gap-ohlone-route-segment-mileage-transcription")
    assert "printed mileage components" in gap.question
    assert "topology is connected end to end" in gap.reason
